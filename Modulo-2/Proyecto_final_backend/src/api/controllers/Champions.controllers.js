...
//? ---------------------------------------------------------------------------------------------------------
//! 83.- Función DELETE para eliminar un objeto de la DB
//? ---------------------------------------------------------------------------------------------------------

const deleteChampion = async (req, res, next) => {
    try {
        //! 84.- Vamos a buscar el id del champion por los params, a controlar si se ha encontrado y si se ha borrado posteriormente
        //! Además, vamos a comprobar si el champion estaba almacenado en alguna región para también eliminarlo de esta

        const {id} = req.params;
        const champion = await Champion.findByIdAndDelete(id);
        const imgToDelete = champion.image;
        deleteImgCloudinary(imgToDelete);

        if(champion) {
            //! 85.- Como ya se ha borrado, en caso de encontrar alguno, dará error en el return
            const findByIdChampion = await Champion.findById(id);
            ...
            //* FALTARÍA POR IMPLEMENTAR EL UPDATE DE LOS USERS

                return res.status(findByIdChampion ? 404 : 200).json({
                    deleteTest: findByIdChampion ? false : true,
                });
            } catch (error) {
                // Error en el update de region
                return res.status(404).json({
                    error: "❌ Error en el update de regiones ❌",
                    message: error.message,
                });
            }
        }
        ...
        // Error general en el delete
        return res.status(404).json({
            error: "❌ Error en el borrado del champion ❌",
            message: error.message,
        })
    }
}
